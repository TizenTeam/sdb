...
/** \file
  This file consists of implementation of SdbInterfaceEnumObject class that
...
SdbInterfaceEnumObject::SdbInterfaceEnumObject()
    : SdbObjectHandle(SdbObjectTypeInterfaceEnumerator) {
...
bool SdbInterfaceEnumObject::InitializeEnum(GUID class_id,
...
bool SdbInterfaceEnumObject::Next(SdbInterfaceInfo* info, ULONG* size) {
  ...
  SdbInstanceEnumEntry& entry = *current_interface_;
...
bool SdbInterfaceEnumObject::Reset() {
  // Make sure that it's opened
  if (!IsOpened()) {
    SetLastError(ERROR_INVALID_HANDLE);
    return false;
