...
#ifndef ANDROID_USB_SDB_API_PRIVATE_DEFINES_H__
#define ANDROID_USB_SDB_API_PRIVATE_DEFINES_H__
/** \file
...
*/
class SdbInstanceEnumEntry {
 public:
  /** \brief Constructs an empty object.
  */
  SdbInstanceEnumEntry()
      : flags_(0) {
  ...
  */
  SdbInstanceEnumEntry(const SdbInstanceEnumEntry& proto) {
    Set(proto.device_name().c_str(), proto.class_id(), proto.flags());
  }

  /** \brief Constructs the object with parameters.
  */
  SdbInstanceEnumEntry(const wchar_t* dev_name, GUID cls_id, DWORD flgs) {
  ...
  */
  ...
  /// Operator =
  SdbInstanceEnumEntry& operator=(const SdbInstanceEnumEntry& proto) {
    Set(proto.device_name().c_str(), proto.class_id(), proto.flags());
    return *this;
  }
  ...
  /// Calculates memory size needed to save this entry into SdbInterfaceInfo
  /// structure
  ULONG GetFlatSize() const {
    return static_cast<ULONG>(FIELD_OFFSET(SdbInterfaceInfo, device_name) +
                              (device_name_.length() + 1) * sizeof(wchar_t));
  }

  /** \brief Saves this entry into SdbInterfaceInfo structure.
  ...
  */
  void Save(SdbInterfaceInfo* info) const {
...
typedef std::vector< SdbInstanceEnumEntry > SdbEnumInterfaceArray;

#endif  // ANDROID_USB_SDB_API_PRIVATE_DEFINES_H__
