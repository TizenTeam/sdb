...
#ifndef ANDROID_USB_API_SDB_API_LEGACY_H_
#define ANDROID_USB_API_SDB_API_LEGACY_H_
/** \file
...
#define SDB_CTL_GET_USB_DEVICE_DESCRIPTOR         10

/// Control code for IOCTL that gets USB_CONFIGURATION_DESCRIPTOR
#define SDB_CTL_GET_USB_CONFIGURATION_DESCRIPTOR  11

/// Control code for IOCTL that gets USB_INTERFACE_DESCRIPTOR
#define SDB_CTL_GET_USB_INTERFACE_DESCRIPTOR      12

/// Control code for IOCTL that gets endpoint information
#define SDB_CTL_GET_ENDPOINT_INFORMATION          13

/// Control code for bulk read IOCTL
#define SDB_CTL_BULK_READ                         14

/// Control code for bulk write IOCTL
#define SDB_CTL_BULK_WRITE                        15

/// Control code for IOCTL that gets device serial number
#define SDB_CTL_GET_SERIAL_NUMBER                 16

/// IOCTL that gets USB_DEVICE_DESCRIPTOR
#define SDB_IOCTL_GET_USB_DEVICE_DESCRIPTOR \
              CTL_CODE(FILE_DEVICE_UNKNOWN, \
                       SDB_CTL_GET_USB_DEVICE_DESCRIPTOR, \
                       METHOD_BUFFERED, \
                       FILE_READ_ACCESS)

/// IOCTL that gets USB_CONFIGURATION_DESCRIPTOR
#define SDB_IOCTL_GET_USB_CONFIGURATION_DESCRIPTOR \
              CTL_CODE(FILE_DEVICE_UNKNOWN, \
                       SDB_CTL_GET_USB_CONFIGURATION_DESCRIPTOR, \
                       METHOD_BUFFERED, \
                       FILE_READ_ACCESS)

/// IOCTL that gets USB_INTERFACE_DESCRIPTOR
#define SDB_IOCTL_GET_USB_INTERFACE_DESCRIPTOR \
              CTL_CODE(FILE_DEVICE_UNKNOWN, \
...
#define SDB_IOCTL_GET_ENDPOINT_INFORMATION \
              CTL_CODE(FILE_DEVICE_UNKNOWN, \
                       SDB_CTL_GET_ENDPOINT_INFORMATION, \
                       METHOD_BUFFERED, \
                       FILE_READ_ACCESS)

/// Bulk read IOCTL
#define SDB_IOCTL_BULK_READ \
              CTL_CODE(FILE_DEVICE_UNKNOWN, \
                       SDB_CTL_BULK_READ, \
                       METHOD_OUT_DIRECT, \
                       FILE_READ_ACCESS)

// For bulk write IOCTL we send request data in the form of SdbBulkTransfer
...
#define SDB_IOCTL_GET_SERIAL_NUMBER \
              CTL_CODE(FILE_DEVICE_UNKNOWN, \
                       SDB_CTL_GET_SERIAL_NUMBER, \
                       METHOD_BUFFERED, \
                       FILE_READ_ACCESS)

///@}

/** Structure SdbQueryEndpointInformation formats input for
  SDB_IOCTL_GET_ENDPOINT_INFORMATION IOCTL request
*/
struct SdbQueryEndpointInformation {
  /// Zero-based endpoint index for which information is queried.
  /// See SDB_QUERY_BULK_xxx_ENDPOINT_INDEX for shortcuts.
  UCHAR endpoint_index;
};

/** Structure SdbBulkTransfer formats parameters for SDB_CTL_BULK_READ and
  SDB_CTL_BULK_WRITE IOCTL requests.
*/
struct SdbBulkTransfer {
  /// Time in milliseconds to complete this request
  ULONG time_out;

  /// Size of the data to transfer. This parameter is used only for
  /// SDB_CTL_BULK_WRITE request. For SDB_CTL_BULK_READ requests transfer
  /// size is defined by the output buffer size.
  ULONG transfer_size;

  /// Initializes statically allocated structure
  __forceinline SdbBulkTransfer() {
    time_out = 0;
    transfer_size = 0;
    for_x64 = 0;
  }
...
protected:
  /// Pointer to the actual buffer for SDB_CTL_BULK_WRITE request. This field
  /// is not used in SDB_CTL_BULK_READ request. Note that in order to support
...
#endif  // ANDROID_USB_API_SDB_API_LEGACY_H_
