...
#ifndef ANDROID_USB_API_SDB_ENDPOINT_OBJECT_H__
#define ANDROID_USB_API_SDB_ENDPOINT_OBJECT_H__
/** \file
  This file consists of declaration of class SdbEndpointObject that
  encapsulates a handle opened to an endpoint on our device.
*/

#include "sdb_interface.h"

/** Class SdbEndpointObject encapsulates a handle opened to an endpoint on
  our device.

  This class implement functionality that is common for both, WinUsb and
  legacy APIs.
*/
class SDBWIN_API_CLASS SdbEndpointObject : public SdbObjectHandle {
 public:
  /** \brief Constructs the object
    ...
    @param[in] endpoint_index Zero-based endpoint index in the interface's
          array of endpoints.
  */
  SdbEndpointObject(SdbInterfaceObject* parent_interf,
                    UCHAR endpoint_id,
 ...
 public:
  /** \brief Gets information about this endpoint.
    ...
    @return true on success, false on failure. If false is returned
            GetLastError() provides extended error information.
  */
  virtual bool GetEndpointInformation(SdbEndpointInformation* info);
  ...
  /** \brief Reads from opened I/O object asynchronously
    ...
    @return A handle to IO completion object or NULL on failure. If NULL is
            returned GetLastError() provides extended error information.
  */
  virtual SDBAPIHANDLE AsyncRead(void* buffer,
                                 ULONG bytes_to_read,
                                 ULONG* bytes_read,
                                 HANDLE event_handle,
                                 ULONG time_out);
  ...
  /** \brief Writes to opened I/O object asynchronously
    ...
    @return A handle to IO completion object or NULL on failure. If NULL is
            returned GetLastError() provides extended error information.
  */
  virtual SDBAPIHANDLE AsyncWrite(void* buffer,
 ...
 public:
  /// This is a helper for extracting object from the SdbObjectHandleMap
  static SdbObjectType Type() {
    return SdbObjectTypeEndpoint;
  }

  /// Gets parent interface 
  SdbInterfaceObject* parent_interface() const {
  ...
  SDBAPIHANDLE GetParentInterfaceHandle() const {
    return (NULL != parent_interface()) ? parent_interface()->sdb_handle() :
                                          NULL;
  }

 protected:
  /// Parent interface
  SdbInterfaceObject* parent_interface_;
...
#endif  // ANDROID_USB_API_SDB_ENDPOINT_OBJECT_H__
